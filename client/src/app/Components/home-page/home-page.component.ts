...
export class HomePageComponent implements OnInit {
  ...
  getMenuItemsByCategory() {
    this.arrayMenuItems = [];
    this.menuItemsService.getMenuItemsByCategory(this.categorySelected).then(
      (res) => {
  ...
  addToCart(item) {
    ...
    localStorage.setItem(
      "arrayCart",
      JSON.stringify(this.arrayItemsAddedToCart)
    );
    this.showSuccess("Item added to cart");
  }
  deleteMenuItem() {
    this.menuItemsService.deleteItem(this.itemSelected.id).then(
      (res) => {
        console.log(res);
        this.getMenuItemsByCategory();
      },
