...
export class RestaurantsComponent implements OnInit {
    ...
    this.restaurantsService.getAllRestaurants()
      .then((restaurants) => {
        ...
        for (var i = 0; i < this.arrayRestaurants.length; i++) {
          ...
          this.objDestination.address ="Street " + this.arrayRestaurants[i].street + ", "+ this.arrayRestaurants[i].streetNumber;
 ...
 delete(restaurant){
   ...
   this.restaurantsService.deleteRestaurant(this.restaurantSelected.id)
   .then((res)=>{
     console.log(res);
     this.arrayRestaurants = this.arrayRestaurants.filter(restaurant => restaurant.id !==this.restaurantSelected.id)
